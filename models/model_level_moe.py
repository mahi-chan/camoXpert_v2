...
class ModelLevelMoE(nn.Module):
    ...
    def __init__(self, backbone='pvt_v2_b2', num_experts=4, top_k=2,
        ...
        # To use FEDER (Frequency Expert), replace one expert below:
        # Example: Replace UJSCExpert with FEDERFrequencyExpert(self.feature_dims)
        self.expert_models = nn.ModuleList([
            ...
            # FEDERFrequencyExpert(self.feature_dims)  # Alternative: Frequency-Domain Expert
        ])

        expert_names = ["SINet-Style", "PraNet-Style", "ZoomNet-Style", "UJSC-Style"]
        # If using FEDER, update: expert_names = ["SINet-Style", "PraNet-Style", "ZoomNet-Style", "FEDER-Style"]
